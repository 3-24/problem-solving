...
int main(){
    ...
    vector<vector<pair<int, int>>> hparg(n+1, vector<pair<int, int>>{});
    while (m--){
        int start, end, dist;
        cin >> start >> end >> dist;
        graph[start].push_back(make_pair(dist, end));
        hparg[end].push_back(make_pair(dist, start));
    }

    vector<int> dists = dijkstra(x, n, graph);
    vector<int> stsid = dijkstra(x, n, hparg);
    debug(dists);
    debug(stsid);

    int max_val = 0;
    for (int i=1; i<=n; i++){
        max_val = max(dists[i] + stsid[i], max_val);
    }
